...
$where= array('user_id'=> session('userid'),'store_id' => $storeid);
...
<div class="fixed-top header_fixeder">
   ...
   $category = DB::table('resturant_category')->where('vendor_id',$storeid)->get();
    $countdata = count($category);?>
                  <select class="btngroup_select" onchange="geturlproduct(this,<?php echo $storeid;?>)">
                     <option>ALL</option>
                     <?php foreach($category as $key => $category){?>
                     <option id="maincat<?php echo $key;?>" value="<?php echo $category->resturant_cat_id;?>"><span><?php echo $category->cat_name;?></span>
                     </option>
                     <?php }?>
                  </select>
            ...
            <img src="{{ url('frontassets/image/user.png') }}" height="20"><br>
               ...
               <div class="dropdown-menu sm-menu shadow " style="position: fixed;top: 89px;left: 74%;">
                  <a class="dropdown-item" href="{{ route('resturantprofile') }}"><i class="fa fa-user"></i>My Profile</a>
                  <a class="dropdown-item" href="{{ route('resturantorder') }}"><i class="fa fa-user"></i>My Order</a>
...
<div class="modal fade" id="cart123" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    ...
    <?php 
               ...
               $cartinfo = DB::table('cart')->where(array('user_id' => $where,'store_id' => $storeid))->get();
        ...
        <div class="col-12">
            <div class="table-responsive" style="overflow-x: hidden;">
                ...
                <table class="table table-striped" >
                    ...
                    <tbody>
                    <?php        
                  foreach($cartinfo as $cartkey => $cartdata) {
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="width: 40%;">
                            <button class="inner_one2 inner_oneuniquedata{{ $cartkey }}" onclick="getminusdata({{ $productdetails->price }},{{ $cartdata->cart_id }},{{ $cartkey }})" ><i class="fas fa-minus inner_one_i2" ></i></button>
                            ...
                            <button class="inner_three2 inner_oneuniquedata{{ $cartkey }}"  onclick="getplusdata('<?php echo $productdetails->price?>','<?php echo $cartdata->cart_id?>','<?php echo $cartkey;?>')" ><i class="fas fa-plus inner_three_i2" ></i></button>
                            </td>
                            <td class="text-right" style="font-size: 13px;font-weight: 500;width: 20%;" id="headerprice<?php echo $cartkey;?>">{{ $currency->currency_sign }}.
                            <?php echo $totprice = $cartdata->qty * $productdetails->price;?></td>
        ...
        <div class="col mb-2">
            <div class="row">
                ...
                <a href="{{ route('resturantcart',$storeid) }}">
                ...
                <a href="{{ route('resturantcheckout',$storeid) }}">
...
<script type="text/javascript">
  ...
  function  geturlproduct(the,storeid) {
     //local
    var cat_id = $(the).val();

    var url = "https://thecodecafe.in/hybrid_resturant/resturantproductdetail/"+cat_id+"?storeid="+storeid+"";
  ...
  function getminusdata(baseprice,cartid,count)
  {
            ...
            $.ajax({
               ...
               success: function(result){
                if(result == 1)
                {
                    ...
                    $('#headerprice'+count).html('Rs. '+total);
                }else 
                {
   ...
   function getplusdata(baseprice,cartid,count)
   {
